...
elseif (!((@include DIR_FS_SMARTY . 'catalog/templates/' . SELECTED_TPL . '/php/' . FILENAME_SHOPPING_CART) == 'overwrite_all')) :
  ...
  if ($_SESSION['cart']->count_contents() > 0) {  
    ...
    $tax_address_query = $DB->prepare
    (
     "SELECT    ab.entry_country_id,
                ab.entry_zone_id
      FROM      " . TABLE_ADDRESS_BOOK . " ab
      LEFT JOIN " . TABLE_ZONES . " z
      ON        (
                ab.entry_zone_id = z.zone_id
                )
      WHERE     ab.customers_id = :customer_id
      AND       ab.address_book_id = :address_book_id"
    );

    $DB->perform($tax_address_query, array(':customer_id' => (int)$_SESSION['customer_id'],
                                           ':address_book_id' => (int)($_SESSION['cart']->get_content_type() == 'virtual' ? $_SESSION['billto'] : $_SESSION['sendto'])));
                                          
    $tax_address = $tax_address_query->fetch();    
