...
elseif (!((@include DIR_FS_SMARTY . 'catalog/templates/' . SELECTED_TPL . '/php/' . FILENAME_PRODUCT_INFO) == 'overwrite_all')) :  
      ...
      if (xos_not_null($products_image_name)) {
        ...
        foreach($products_image_name as $products_img_name) {    
          $products_images_array[]=array('link_product_img' => xos_href_link(FILENAME_POPUP_IMAGE, 'pID=' . $product_info['products_id'] . '&img_name='. rawurlencode($products_img_name['name'])),
                                         ...
                                         'product_img_medium' => xos_image(DIR_WS_IMAGES . 'products/medium/' . rawurlencode($products_img_name['name']), addslashes($product_info['products_name']), '', '', 'class="info-img"'),
