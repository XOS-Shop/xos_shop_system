    ...
    function calc_image_size($width, $height) {
      ...
      if ($this->max_width > 0 && $this->max_width <= $width) {
        ...
        if ($this->max_height > 0) {
          if ($new_size[1] > $this->max_height) $new_size = $this->calc_height($new_size[0], $new_size[1]);
        }    
...
//          $new_image = ImageCreate($new_size[0], $new_size[1]); // Transparenz wenn auskommentiert aber schlechte Bildqualitaet
        ...
        case "JPEG":
          ...
          // sharpen setting for thumbnail smaller 300 pixel
          if ($new_size[0] < 300 || $new_size[1] < 300) {
