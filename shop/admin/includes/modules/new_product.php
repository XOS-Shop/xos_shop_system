...
if (!((@include DIR_FS_SMARTY . 'admin/templates/' . ADMIN_TPL . '/php/includes/modules/new_product.php') == 'overwrite_all')) :  
    ...
    while ($customers_group = xos_db_fetch_array($customers_group_query)) { 
      ...
      $customers_groups_array[]=array('name' => $customers_group['customers_group_name'],
                                      ...
                                      'input_special_expires_date' => xos_draw_input_field('special_expires_date_' . $customers_group['customers_group_id'], $special_expires_date['expires_date'], 'id ="special_expires_date_' . $customers_group['customers_group_id'] . '" style="background: #ffffcc;" size ="10" autocomplete="off"'),                                                                                
...
//                   "\n" . '  $( "#ui-datepicker-div" ).css( "font-size", "75%" );' . "\n\n" .
    ...
    if (WYSIWYG_FOR_PRODUCT == 'true') {      
      $smarty->assign(array('wysiwyg' => true,
                            ...
                            'product_config' => (ENABLE_SSL == 'true' ? ($_SESSION['disable_ssl'] ? HTTP_SERVER : HTTPS_SERVER) : HTTP_SERVER) . DIR_WS_ADMIN_IMAGES . ADMIN_TPL . '/ckconfig/product_config.js',
                            'product_base_href' => ENABLE_SSL == 'true' ? ($_SESSION['disable_ssl'] ? HTTP_SERVER : HTTPS_SERVER) : HTTP_SERVER,                            
    ...
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
      ...
      $product_values[]=array('languages_image' => xos_image(DIR_WS_CATALOG_IMAGES . 'catalog/templates/' . DEFAULT_TPL . '/' . $languages[$i]['directory'] . '/' . $languages[$i]['image'], $languages[$i]['name']),
                              ...
                              'product_info_template_file' => (ENABLE_SSL == 'true' ? ($_SESSION['disable_ssl'] ? HTTP_SERVER : HTTPS_SERVER) : HTTP_SERVER) . DIR_WS_ADMIN . 'includes/ckconfig/templates/' . $languages[$i]['directory'] . '/product_info_template.js',
                              'product_info_template_lang' => $languages[$i]['directory'] . '_default',
                              'product_description_template_file' => (ENABLE_SSL == 'true' ? ($_SESSION['disable_ssl'] ? HTTP_SERVER : HTTPS_SERVER) : HTTP_SERVER) . DIR_WS_ADMIN . 'includes/ckconfig/templates/' . $languages[$i]['directory'] . '/product_description_template.js',
    ...
    $smarty->assign(array('javascript' => $javascript,
                          ...
                          'input_products_date_available' => xos_draw_input_field('products_date_available', $pInfo->products_date_available, 'id="products_date_available" style="background: #ebebff; color : red;" size ="10" autocomplete="off"'),
