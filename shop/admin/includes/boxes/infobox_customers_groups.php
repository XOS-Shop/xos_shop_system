  ...
  switch ($action) {
    case 'confirm':
      if ($_GET['cID'] != '0') {
        ...
        $contents[] = array('text' => '<br /><a href="" onclick="customers_groups.submit(); return false" class="btn btn-danger btn-margin-infobox" title=" ' . BUTTON_TITLE_DELETE . ' ">' . BUTTON_TEXT_DELETE . '</a><a href="' . xos_href_link('customers_groups.php', xos_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_group_id) . '" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_CANCEL . ' ">' . BUTTON_TEXT_CANCEL . '</a><br />&nbsp;');
      } else {
        $heading_title = ''. xos_draw_separator('pixel_trans.gif', '11', '12') .'&nbsp;<br /><b>' . TEXT_INFO_HEADING_DELETE_GROUP . '</b>';
    ...
    default:
      if (is_object($cInfo)) {
        ...
        $contents[] = array('text' => '<a href="' . xos_href_link('customers_groups.php', xos_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_group_id . '&action=edit') . '" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_EDIT . ' ">' . BUTTON_TEXT_EDIT . '</a><a href="' . xos_href_link('customers_groups.php', xos_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_group_id . '&action=confirm') . '" class="btn btn-danger btn-margin-infobox" title=" ' . BUTTON_TITLE_DELETE . ' ">' . BUTTON_TEXT_DELETE . '</a><br />&nbsp;');
      }
      break;
  }
