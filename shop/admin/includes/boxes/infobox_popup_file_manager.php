...
if (!((@include DIR_FS_SMARTY . 'admin/templates/' . ADMIN_TPL . '/php/includes/boxes/infobox_popup_file_manager.php') == 'overwrite_all')) :
    ...
    switch ($action) {
      case 'delete':
        ...
        $contents[] = array('text' => '<br /><a href="" onclick="file.submit(); return false" class="btn btn-danger btn-margin-infobox" title=" ' . BUTTON_TITLE_DELETE . ' ">' . BUTTON_TEXT_DELETE . '</a><a href="' . xos_href_link(FILENAME_POPUP_FILE_MANAGER, (xos_not_null($fInfo->name) ? 'info=' . urlencode($fInfo->name) : '')) . '" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_CANCEL . ' ">' . BUTTON_TEXT_CANCEL . '</a><br />&nbsp;');
        break;
      case 'new_folder':
        ...
        $contents[] = array('text' => '<br />' . (($directory_writeable == true) ? '<a href="" onclick="folder.submit(); return false" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_SAVE . ' ">' . BUTTON_TEXT_SAVE . '</a>' : '') . '<a href="' . xos_href_link(FILENAME_POPUP_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')) . '" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_CANCEL . ' ">' . BUTTON_TEXT_CANCEL . '</a><br />&nbsp;');
        break;
      case 'upload':
        ...
        $contents[] = array('text' => '<br />' . (($directory_writeable == true) ? '<a href="" onclick="file.submit(); return false" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_UPLOAD . ' ">' . BUTTON_TEXT_UPLOAD . '</a>' : '') . '<a href="' . xos_href_link(FILENAME_POPUP_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')) . '" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_CANCEL . ' ">' . BUTTON_TEXT_CANCEL . '</a><br />&nbsp;');
        break;
      default:
        if (isset($fInfo) && is_object($fInfo)) {
          ...
          if (!($fInfo->is_dir || $fInfo->is_image)) {
            $contents[] = array('text' => ($_SESSION['link_entrence'] ? '<a href="' . xos_href_link(FILENAME_POPUP_FILE_MANAGER, 'action=link_entrence') . '" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_BACK_TO_OVERVIEW . ' ">' . BUTTON_TEXT_BACK_TO_OVERVIEW . '</a>' : '') . '<a href="' . xos_href_link(FILENAME_POPUP_FILE_MANAGER, 'info=' . urlencode($fInfo->name) . '&action=edit') . '" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_EDIT . ' ">' . BUTTON_TEXT_EDIT . '</a><a href="" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_SELECT . ' " onclick="window.opener.CKEDITOR.tools.callFunction(\'' . $_SESSION['ckeditor_func_num'] . '\', \'' . $ws_path . $fInfo->name . '\', \'\');window.close();window.opener.focus();return false;">' . BUTTON_TEXT_SELECT . '</a>');
          }
          if ($fInfo->is_dir && $_SESSION['link_entrence']) {
            $contents[] = array('text' => '<a href="' . xos_href_link(FILENAME_POPUP_FILE_MANAGER, 'action=link_entrence') . '" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_BACK_TO_OVERVIEW . ' ">' . BUTTON_TEXT_BACK_TO_OVERVIEW . '</a>');
          }
          if ($fInfo->is_image) {
            $contents[] = array('text' => ($_SESSION['link_entrence'] ? '<a href="' . xos_href_link(FILENAME_POPUP_FILE_MANAGER, 'action=link_entrence') . '" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_BACK_TO_OVERVIEW . ' ">' . BUTTON_TEXT_BACK_TO_OVERVIEW . '</a>' : '') . '<a href="' . xos_href_link(FILENAME_POPUP_FILE_MANAGER, 'info=' . urlencode($fInfo->name) . '&action=view') . '" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_REAL_IMAGE . ' ">' . BUTTON_TEXT_REAL_IMAGE . '</a><a href="" class="btn btn-default btn-margin-infobox" title=" ' . BUTTON_TITLE_SELECT . ' " onclick="window.opener.CKEDITOR.tools.callFunction(\'' . $_SESSION['ckeditor_func_num'] . '\', \'' . $ws_path . $fInfo->name . '\', \'\');window.close();window.opener.focus();return false;">' . BUTTON_TEXT_SELECT . '</a>');
