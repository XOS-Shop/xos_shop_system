        ...
        if ($content_error == false) {
          ...
          for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
            $sql_data_array = array('name' => xos_db_prepare_input(htmlspecialchars_decode($_POST['name'][$languages[$i]['id']])),         
                                    ...
                                    'content' => preg_replace_callback(array('#href=\"?(([^\" >]*?)(\.php)([^\" >]*?))#siU','#href=\"?(([^\" >]*?)(\.html/[a-rz])([^\" >]*?))#siU','#href=\"?(([^\" >]*?)(\.[a-rz])([^\" >]*?))#siU'), 'internal_link_replacement', (trim(str_replace('&#160;', '', strip_tags(xos_db_prepare_input($_POST['content'][$languages[$i]['id']]), '<img>'))) != '') ? xos_db_prepare_input($_POST['content'][$languages[$i]['id']]) : ''),
