        ...
        if ($category_error == false) {
          ...
          for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
            ...
            $sql_data_array = array('categories_or_pages_name' => xos_db_prepare_input(htmlspecialchars_decode($categories_or_pages_name_array[$language_id])),         
                                    ...
                                    'categories_or_pages_content' => preg_replace_callback(array('#href=\"?(([^\" >]*?)(\.php)([^\" >]*?))#siU','#href=\"?(([^\" >]*?)(\.html/[a-rz])([^\" >]*?))#siU','#href=\"?(([^\" >]*?)(\.[a-rz])([^\" >]*?))#siU'), 'internal_link_replacement', (trim(str_replace('&#160;', '', strip_tags(xos_db_prepare_input($categories_or_pages_content_array[$language_id]), '<img>'))) != '') ? xos_db_prepare_input($categories_or_pages_content_array[$language_id]) : ''));
      ...
      case 'update_product':            
          ...
          for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
            ...
            $sql_data_array = array('products_name' => xos_db_prepare_input(htmlspecialchars($_POST['products_name'][$language_id])),
                                    ...
                                    'products_info' => preg_replace_callback(array('#href=\"?(([^\" >]*?)(\.php)([^\" >]*?))#siU','#href=\"?(([^\" >]*?)(\.html/[a-rz])([^\" >]*?))#siU','#href=\"?(([^\" >]*?)(\.[a-rz])([^\" >]*?))#siU'), 'internal_link_replacement', (trim(str_replace('&#160;', '', strip_tags(xos_db_prepare_input($_POST['products_info'][$language_id]), '<img>'))) != '') ? xos_db_prepare_input($_POST['products_info'][$language_id]) : ''),
                                    'products_description' => preg_replace_callback(array('#href=\"?(([^\" >]*?)(\.php)([^\" >]*?))#siU','#href=\"?(([^\" >]*?)(\.html/[a-rz])([^\" >]*?))#siU','#href=\"?(([^\" >]*?)(\.[a-rz])([^\" >]*?))#siU'), 'internal_link_replacement', (trim(str_replace('&#160;', '', strip_tags(xos_db_prepare_input($_POST['products_description'][$language_id]), '<img>'))) != '') ? xos_db_prepare_input($_POST['products_description'][$language_id]) : ''),
        ...
        if (isset($_POST['products_id']) && isset($_POST['categories_or_pages_id'])) {
          ...
          if (xos_children_in_category_count($categories_or_pages_id) > 0) {
            ...
            if ($_POST['copy_as'] == 'link') {
              ...
              $product_query = xos_db_query("select products_quantity, products_delivery_time_id, products_model, products_image, products_price, products_date_available, products_weight, products_tax_class_id, manufacturers_id, attributes_quantity, attributes_combinations from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
              $product = xos_db_fetch_array($product_query);
            
              xos_db_query("insert into " . TABLE_PRODUCTS . " (products_quantity, products_delivery_time_id, products_model, products_image, products_price, products_sort_order, products_date_added, products_date_available, products_weight, products_status, products_tax_class_id, manufacturers_id, attributes_quantity, attributes_combinations) values ('" . xos_db_input($product['products_quantity']) . "', '" . xos_db_input($product['products_delivery_time_id']) . "', '" . xos_db_input($product['products_model']) . "', '" . xos_db_input($product['products_image']) . "', '" . xos_db_input($product['products_price']) . "', '0', now(), " . (empty($product['products_date_available']) ? "null" : "'" . xos_db_input($product['products_date_available']) . "'") . ", '" . xos_db_input($product['products_weight']) . "', '0', '" . (int)$product['products_tax_class_id'] . "', '" . (int)$product['manufacturers_id'] . "', " . (xos_not_null($product['attributes_quantity']) ? "'" .xos_db_input($product['attributes_quantity']) . "'" : "null") . ", " . (xos_not_null($product['attributes_combinations']) ? "'" .xos_db_input($product['attributes_combinations']) . "'" : "null") . ")");
