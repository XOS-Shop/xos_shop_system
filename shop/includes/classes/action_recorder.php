      ...
      if (defined('MODULE_ACTION_RECORDER_INSTALLED') && xos_not_null(MODULE_ACTION_RECORDER_INSTALLED)) {
        if (xos_not_null($module) && in_array($module . '.' . substr($_SERVER['BASENAME_PHP_SELF'], (strrpos($_SERVER['BASENAME_PHP_SELF'], '.')+1)), explode(';', MODULE_ACTION_RECORDER_INSTALLED))) {
          if (!class_exists($module)) {
            if (file_exists(DIR_WS_MODULES . 'action_recorder/' . $module . '.' . substr($_SERVER['BASENAME_PHP_SELF'], (strrpos($_SERVER['BASENAME_PHP_SELF'], '.')+1)))) {
              ...
              include(DIR_WS_MODULES . 'action_recorder/' . $module . '.' . substr($_SERVER['BASENAME_PHP_SELF'], (strrpos($_SERVER['BASENAME_PHP_SELF'], '.')+1)));
            } else {
              return false;
            }
          }
