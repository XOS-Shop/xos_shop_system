...
/* Klasse SplitPageResults fuer PDO */
class SplitPageResultsPDO
{
    var $sql_query, $sql_param, $number_of_rows, $current_page_number, $number_of_pages, $number_of_rows_per_page, $page_name, $offset;
    
    /* class constructor */
    function __construct($query, $max_rows, $count_key = '*', $param = array(), $page_holder = 'page')
    {
        $DB = Registry::get('DB');
        $this->sql_query = $query;
        $this->sql_param = $param;
        ...
        $count_query = $DB->prepare("select count(" . $count_string . ") as total " . substr($this->sql_query, $pos_from, ($pos_to - $pos_from)));
        
        $DB->perform($count_query, $this->sql_param);
        ...
        $this->offset = max(($this->number_of_rows_per_page * ($this->current_page_number - 1)), 0);
        
        $this->sql_query .= " limit :offset, :limit";
        
        $this->sql_param[':offset'] = (int) $this->offset;
        $this->sql_param[':limit']  = (int) $this->number_of_rows_per_page;
    }
    ...
    function display_links($max_page_links, $parameters = '')
    {
        ...
        if ($cur_window_num > 1)
            $display_links_string .= '&nbsp;<a href="' . xos_href_link($_SERVER['BASENAME_PHP_SELF'], $parameters . $this->page_name . '=' . (($cur_window_num - 1) * $max_page_links), $request_type) . '" class="page-results" title=" ' . sprintf(PREVNEXT_TITLE_PREV_SET_OF_NO_PAGE, $max_page_links) . ' "><span class="text-deco-underline">...</span></a>&nbsp;';
        ...
        if ($cur_window_num < $max_window_num)
            $display_links_string .= '&nbsp;<a href="' . xos_href_link($_SERVER['BASENAME_PHP_SELF'], $parameters . $this->page_name . '=' . (($cur_window_num) * $max_page_links + 1), $request_type) . '" class="page-results" title=" ' . sprintf(PREVNEXT_TITLE_NEXT_SET_OF_NO_PAGE, $max_page_links) . ' "><span class="text-deco-underline">...</span></a>&nbsp;';
...
/*
// Klasse SplitPageResults fuer PDO Variation
  class SplitPageResultsPDO {
