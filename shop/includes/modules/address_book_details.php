...
if (!((@include DIR_FS_SMARTY . 'catalog/templates/' . SELECTED_TPL . '/php/includes/modules/address_book_details.php') == 'overwrite_all')) :
  ...
  $smarty->assign(array('input_firstname' => xos_draw_input_field('firstname', $entry['entry_firstname'], 'class="form-control" id="firstname"') . '&nbsp;' . (xos_not_null(ENTRY_FIRST_NAME_TEXT) ? '<span class="input-requirement">' . ENTRY_FIRST_NAME_TEXT . '</span>': ''),
                        ...
                        'input_street_address' => xos_draw_input_field('street_address', $entry['entry_street_address'], 'class="form-control" id="street_address" onblur="if(!/[1-9]/.test(this.value) && this.value.length >= ' . ENTRY_STREET_ADDRESS_MIN_LENGTH . '){$(\'#number-error\').show(100)}else{$(\'#number-error\').hide(100)}"') . '&nbsp;' . (xos_not_null(ENTRY_STREET_ADDRESS_TEXT) ? '<span class="input-requirement">' . ENTRY_STREET_ADDRESS_TEXT . '</span>': '') . '<p id="number-error" style="display: none;"><span class="red-mark">' . ENTRY_MISSING_HOUSE_NUMBE_TEXT_1 . '</span>&nbsp; &nbsp;' . ENTRY_MISSING_HOUSE_NUMBE_TEXT_2 . '</p>',
