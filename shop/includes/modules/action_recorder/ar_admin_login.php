...
////////////////////////////////////////////////////////////////////////////////
  ...
  class ar_admin_login {
    ...
    function canPerform($user_id, $user_name) {
      $check_query = xos_db_query("select id from " . TABLE_ACTION_RECORDER . " where module = '" . xos_db_input($this->code) . "' and (" . (!empty($user_name) ? "user_name = '" . xos_db_input($user_name) . "' or " : "") . " identifier = '" . xos_db_input($this->identifier) . "') and date_added >= date_sub(now(), interval " . (int)$this->minutes  . " minute) and success = 0 order by date_added desc limit " . (int)$this->attempts);
    ...
    function install() {
      xos_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_ACTION_RECORDER_ADMIN_LOGIN_MINUTES', '5', '6', '0', now())");
      xos_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_ACTION_RECORDER_ADMIN_LOGIN_ATTEMPTS', '3', '6', '0', now())");
    }
